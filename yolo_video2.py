...
# RETURN: N/A
def displayVehicleCount(frame,frame1, vehicle_count):
##        cv2.putText(
##                frame, #Image
        ...
        if len(idxs) > 0:
                ...
                for i in idxs.flatten():
                        ...
                        if (LABELS[classIDs[i]] in list_of_vehicles):
                                ...
                                if(LABELS[classIDs[i]]=='bus' or  LABELS[classIDs[i]]=='truck'):
                                        vehicle_count += 1
                                        
                                if(LABELS[classIDs[i]]=='bicycle' or  LABELS[classIDs[i]]=='motorbike'):
                                        vehicle_count -= 1
...
while True:
        ...
        while(iik<2):
                ...
                displayVehicleCount(frame,frame1, vehicle_count)
        ...
        ##                        # of the current object detection
        ##                        scores1 = detection[5:]
        ##                        classID1 = np.argmax(scores)
        ##                        confidence1 = scores[classID]
        ##
        ##                        # filter out weak predictions by ensuring the detected
        ##                        # probability is greater than the minimum probability
        ##                        if confidence1 > preDefinedConfidence:
        ##                                # scale the bounding box coordinates back relative to
        ##                                # the size of the image, keeping in mind that YOLO
        ##                                # actually returns the center (x, y)-coordinates of
        ##                                # the bounding box followed by the boxes' width and
        ##                                # height
        ##                                box1 = detection[0:4] * np.array([video_width, video_height, video_width, video_height])
        ##                                (centerX, centerY, width, height) = box.astype("int")
        ##
        ##                                # use the center (x, y)-coordinates to derive the top
        ##                                # and and left corner of the bounding box
        ...
        ##        # apply non-maxima suppression to suppress weak, overlapping
        ##        # bounding boxes
        ##        idxs1 = cv2.dnn.NMSBoxes(boxes1, confidences1, preDefinedConfidence,
        ##                preDefinedThreshold)
        ##
        ##        # Draw detection box 
        ##        drawDetectionBoxes(idxs1, boxes1, classIDs1, confidences1, frame)
        ##
        ##        vehicle_count, current_detections = count_vehicles(idxs1, boxes1, classIDs1, vehicle_count, previous_frame_detections, frame)
        ##
        ##        # Display Vehicle Count if a vehicle has passed the line 
        ##        displayVehicleCount(frame,frame1, vehicle_count)
        ##
